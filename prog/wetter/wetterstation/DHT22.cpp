...
uint8_t DHT22::DHT22_StartReading(void){
	...
	if (state == DHT_STOPPED || state == DHT_DATA_READY || state == DHT_ERROR_CHECKSUM || state == DHT_ERROR_NOT_RESPOND){
		...
		TIMER_limit = 255; // Timer compare value equals to overflow (interrupt will fired at 255us).
		TIMER_counter = 0; // Reset counter value.
		state = DHT_HOST_START; // Change state.
		TIMER_run_status = true; // Start timer with prescaler such that 1 tick equals 1us (freq = 1MHz).
...
void DHT22::ISR_INT_ROUTINE(){
	uint8_t counter_us;
	counter_us = TIMER_counter; // Store counter value
	...
	if (bitcounter > 39){ // Transfer done
		TIMER_run_status = false;//stop timer
		TIMER_counter = 0; // Reset counter.
		EXT_INTERRUPT_DISABLE // Disabling interrupt
		...
		if(TIMER_overflow_status){
		   ...
		   register to 40. */
			   ...
			   80us. So, we can measure period P3 at the next rising edge interrupt. */
				...
				TIMER_limit = 255; // Change timer compare to 255, for now on, the timer interrupt should not fire.
